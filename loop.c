...
{
	static struct loop_randdiff_data cmd_data;
	...
	switch (command)
	{
		case command_info: {printf("loop_rand_diff\n"); return result_continue;}
		case command_insert:
		{
			memcpy(&cmd_data, command_data, sizeof(struct loop_randdiff_data));
...
{
	static struct loop_randq_data cmd_data;
	...
	switch (command)
	{
		case command_info: {printf("loop_randq\n"); return result_continue;}
		case command_insert:
		{
			memcpy(&cmd_data, command_data, sizeof(struct loop_randq_data));
		...
		case command_run:
		{
			set_rand_q(parameters, index, cmd_data.rho_numerator, cmd_data.rho_denominator);
			return result_continue;
		}
		default: return result_continue;
	}
...
short int loop_randq_equal(short int command, struct loop_randq_data *command_data, void *data[], void *data_parameters[], short int data_size)
{
	...
	switch (command)
	{
		case command_info: {printf("loop_randq_equal\n"); return result_continue;}
		case command_insert:
		{
			memcpy(&cmd_data, command_data, sizeof(struct loop_randq_data));
		...
		case command_run:
		{
			set_rand_q_equal(parameters, index);
			return result_continue;
		}
		default: return result_continue;
	}
